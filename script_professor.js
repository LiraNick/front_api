...
function load_professor(id_professor){
   get_by_id(id_professor).then(professor =>{
       ...
       document.getElementById("materia").value = professor.materia;
   });
...
function save(){
   ...
   if(id != ''){
       console.log('editar')
       professor = {"id":id, "nome": nome, "email": email, "materia":materia}
       update(professor);
   }
   else{
       professor = {"nome": nome, "email": email, "materia":materia}
       create(professor);
   }       
}
